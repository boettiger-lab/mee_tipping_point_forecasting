...
Since ARIMA models make the assumption that the future will resemble the past via their auto-regressive and moving average components [@cite], ARIMA models are not well-suited for tipping point problems.
...
In contrast to inference with MCMC, uncertainty is estimated directly from the learned parameters [@cite].


## Method group 3: Machine Learning models

Over the past decade, deep learning has become very popular for a broad range of challenging predictive tasks [@cite];
However, there have been a relatively small number of studies investigating how deep learning methods perform on sequence to sequence time series forecasting problems [@kao, @lyu, @Du].
Across these studies, it has become established that using memory or attention-based neural networks, and an encoder-decoder architecture is important for improving forecasting performance. 
...
Machine learning models are often used to make point forecasts, yet for their application to complex time series problems like tipping points, it will be necessary to use methods that estimate uncertainty. 
...
To estimate uncertainty intervals, we simply draw samples from the learned distribution at each time step and report the 95% confidence interval.
...
A proposed reason for this instability is that the machine learning models often get stuck on local optima of loss surfaces [@modelstability].
To regularize against the common initialization instability problem, each machine learning forecast in [@figures] is an ensemble forecast over 5 different initialization seeds.
