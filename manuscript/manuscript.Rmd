...
All though most ML-based forecasting applications focus on point predictions, we have emphasized examples which can provide a meaningful estimate of uncertainty.
When machine learning models are able to observe transition phenomena, as in the stochastic case, they performed comparably to MCMC-based forecasting with respect to CRPS and log probability score but under-performed MCMC when there were no transition events in the training sets as in the Hopf and Saddle-node examples.
...
Yet, there are many nuances to these assessments as examining summary statistics, CRPS and log probability scores obscure finer detailed components of the forecasts.
For instance, forecast skill varies with the length of the forecast horizon, and not in a monotonic fashion. 
This is the result of multiple factors: for some dynamics, such as those involving tipping points, the long term behavior can be easier to predict then transient transitions. 
...
All methods are given only a single previous replicate of a stochastic transition (Fig 5, red line) on which to base there estimates.
...
This provides a clear illustration of the much broader array of phenomenological behaviors that can be accurately models with ML models compared to classical statistical models.
...
This question is not as easy to answer as it may seem, because of the difficulty in defining what real-world forecasting scenario that may correspond to.
...
Would it be possible to train an predictive algorithm on 'analogous' examples of critical transitions?
...
Even if the underlying mechanism was accurately captured by the same model, say, the saddle-node model of @May1977 we consider here, it is likely that most of the individual model parameters would be quite different.



Identifiability MCMC theoretical limits
...
Neither can we claim that the ML methods employed will translate well to all sudden transition event forecasting problems in reality, since working with real data will introduce additional difficulties.
Some potential issues include varying state and time scales across time series, non-concurrent sampling, sparse data, and more.
