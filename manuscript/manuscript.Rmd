...
Since ARIMA models make the assumption that the future will resemble the past via ARIMA's auto-regressive and moving average components [@cite], ARIMA models are not well-suited for tipping point problems.
Thus, ARIMA-based forecasts should serve as a lower threshold of acceptability for the performance of non-mechanistic models.
...
Across these studies, it has become established that using memory or attention-based neural networks, and an encoder-decoder architecture is crucial for improving forecasting performance.
...
Machine learning models are often used to make point forecasts, yet for their application to ecological time series, it will be necessary to use methods that estimate uncertainty. 
...
To estimate uncertainty intervals, we draw samples from the learned distribution at each time step and report the 95% confidence interval.
...
One proposed reason for this instability is that machine learning models often get stuck on the local optima of loss surfaces [@modelstability]. 
Another likely reason for this instability is overfitting to the training set.
Overfitting is a fundamental issue in deep learning, for which there is no universal solution [@ying].
A common method to reduce overfitting is to perform K-fold cross validation [@ying], but this approach cannot be effectively employed when there is one or few time series in the training set.
To remedy the instability problem, we use an ensemble model, wherein each ML forecast is the union of forecasts from 5 models that were trained with different initialization seeds.
We found this simple ensemble method to be an effective way to improve the generalizability for each ML model.
...
It is worth noting that this property applies specifically to probabilistic forecasts, and is not true of all metrics often used to compare forecasts.
