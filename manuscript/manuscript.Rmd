...
The Nicholson-Bailey model is a two species model which contains a super-critical Hopf bifurcation, a non-catastrophic bifurcation which either creates or destroys a limit cycle -- a stable oscillatory pattern. 
...
```{r fig.cap="Forecast scenarios. A. The Hopf bifurcation: a stable node develops into a limit cycle. B. The saddle-node bifurcation. C. The stochastic transition."}
...
Following @dakos_methods_2012, we further allow the carrying capacity of the host, $K$ to slowly increase at a linear rate, which drives a super-critical Hopf bifurcation as $K$ becomes sufficiently large. 
...
#| gradually in the case of Hopf bifurcation or suddenly in response to the saddle-node bifurcation. 
...
Although most ML-based forecasting applications focus on point predictions, we have emphasized examples that can provide a meaningful estimate of uncertainty.
