...
While 'tactical' population dynamics models used to inform decision making in fields such as fisheries are often much more complex, these simple models nevertheless exhibit rich nonlinear dynamics thought to be typical in ecological systems [@]. <!-- MFL: Using 'tactical' might throw readers off that aren't familiar with tact vs strat distinction -->
...
These dynamics have long been identified as an acute challenge to forecasting, with effort focused primarily on identifying any 'early warning signs' that a catastrophic bifurcation might occur at all [@Scheffer2009; @Scheffer2012] rather than more ambitious attempts to provide quantitative probabilistic forecasts of the likely distribution of waiting times before such a transition occurs. <!-- MFL: I think we can make the first half of this sentence clearer. I think some readers might get lost here. -->
...
Over the past decade, deep learning has become very popular for a broad range of challenging time series prediction problems [@kao, @lyu, @du].
Across these studies, it has become established that using memory or attention-based neural networks, and an encoder-decoder architecture is crucial for improving forecasting performance -- see @kao, @lyu and @du for more context on memory, attention and encoder-decoders.
Memory and attention are proposed as advantageous for time series forecasting since these methods enable models to learn long term dependencies [@kao, @attentionisall].
Encoder-decoder architectures have been shown to excel on sequence to sequence tasks [@learningphase].
To investigate these assertions on the challenging class of tipping point problems, we examine four models that use a combination of memory, attention and encoder-decoder network architectures.
We will examine LSTM and GRU models, which are both memory-based networks, Transformers, which use attention with an encoder-decoder, and Block LSTM's, which employ LSTM's in an encoder-decoder framework.

Machine learning models are often used to make point forecasts, yet for their application to ecological time series, it will be necessary to use probablisitic forecasts. 
For all the machine learning models in this study, we use the same method for uncertainty estimation:
each model is trained to learn the parameters of a poisson distribution that maximizes the log likelihood. 
...
Overfitting is a fundamental issue in deep learning, for which there is no universal solution [@ying].
...
We found this simple ensemble method to be an effective way to improve generalizability.
...
In other words, while it is possible for any of the models considered to _overfit_ the data against which they are trained, i.e. have a higher likelihood than the true process, it is not possible for these models to overfit the data against which they are scored. <!-- MFL: I think we can take out the parenthetical here and assume the reader knows what we mean by overfitting-->
It is worth noting that this property applies specifically to probabilistic forecasts, and is not true of all metrics often used to compare forecasts. <!-- MFL: hmm I am not completely following this point here -->
Concerns about over-fitting arise in most types of model estimation and are a particularly acute concern to machine learning methods due to the bias-variance trade-off [@]. <!-- MFL: I don't think I nailed the overfitting part in ML section but we probably want to reference this here-->
...
mcmc_scores <- bind_rows(
  read_csv("../greta/scores/scores_saddlenode.csv.gz") |> mutate(scenario="saddlenode"),
read_csv("../greta/scores/scores_stochastic.csv.gz"),
read_csv("../greta/scores/scores_hopf.csv.gz") |> 
