...
```{r}
shift |> 
  ggplot(aes(forecasting_model, score, color=forecasting_model, fill=forecasting_model)) + 
  geom_violin() +
  facet_grid(metric ~ simulation, scales = "free") + 
  scale_y_log10() +
  theme_bw() + 
  coord_flip()
  
  
  ggplot(aes(simulation, score, color=forecasting_model, fill=forecasting_model)) + 
  geom_violin() +
...
```{r}
# scores over time, averaged over replicates
over_time <- shift |> 
  group_by(t, simulation,forecasting_model, metric ) |> 
  summarise(score = mean(score)) 

over_time |> 
  ggplot(aes(t, score, col = forecasting_model)) + 
  geom_point(size=1) +
  facet_grid(metric ~ simulation,
             scales="free") + 
