...
Across these studies, it has become established that using memory or attention-based neural networks, and an encoder-decoder architecture is crucial for improving forecasting performance -- see @kao, @lyu and @du for more details on these concepts.
...
Another likely reason is that machine learning models commonly overfit the training data.
Overfitting is a fundamental issue in deep learning, for which there is no universal solution [@ying].
One frequently cited method to reduce overfitting is K-fold cross validation [@ying], but this approach cannot be effectively employed when there is one or few time series in the training set.
