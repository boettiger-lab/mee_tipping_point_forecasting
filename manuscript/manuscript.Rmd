---
title: "Ecological Limits to Forecasting: Estimating Uncertainty in Critical Transitions with Deep Learning"
authors:
...
Forecasting plays an important and rapidly growing role in both testing fundamental understanding of ecological processes and informing ecological applications and conservation decision-making [@dietze_iterative_2018; @schindler_portfolio_2015].
Meanwhile, rapid advances in machine learning have rapidly improved the prevalence and accuracy of short term forecasts in many fields [@something]. 
Will these emerging methods improve the capacity for forecasts in ecological systems as well?
Ecological dynamics are notoriously complex, with uncertainty and non-linearity playing critical roles [@Boettiger2018; @hallett_why_2004; @Oveskienen2010].
These challenges are nowhere more evident then in _critical transitions_, sudden shifts in the states or patterns of ecosystem dynamics that are simultaneously more important and more difficult to predict than more gradual changes.
...
Robert May's consumer-resource model [@May1977], and the Nicholson-Bailey parasatoid-host model [@nicholson_balance_1935]. 
...
This can create a particularly challenging forecasting task when such transitions have not been previously observed in the same system, requiring the forecast to anticipate dynamics for which there are no corresponding analog in the historical data to date. 
...
Thus our examples of scoring and visualization will rely on a collection of R packages, in particular, `scoringRules` for the efficient calculation of Continuous Ranked Probability Score (CRPS) and logarithmic probability (Logs) scores for forecast ensembles [@gneiting_strictly_2007].
...
The Nicholson-Bailey model describes a predator-prey dynamic for the relationship of a host species and an obligate parsatiod, originally used to model the population dynamics of blowflies (_Lucilia cuprina_) [@nicholson_balance_1935; @nicholson_outline_1954; @nicholson_compensatory_1954].
...
Following @dakos_methods_2012, we further allow the carrying capacity of the host, $K$ to slowly increase at linear rate, which drives a Hopf bifurcation as $K$ becomes sufficiently large. 
...
May's consumer-resource model is a one-dimensional model describing the growth of a 'resource' population (e.g. herbivore) which is grazed by a consumer [@may_population_1979].
...
For a certain range of parameter choices, this model supports alternative stable state dynamics, and this model has been identified and employed in explaining alternative stable state dynamics in a broad range of ecological and socio-ecological systems [@scheffer_catastrophic_2001].  
...
Since ARIMA is a very commonly encountered forecasting method, we will refer readers to @hyndman_forecasting_2018 for further discussion.
...
These methods avoid gradient problems by regulating the self-feedback between time steps via gates which perform operations on the feedback signal -- see @chung_empirical_2014 for more details.
...
Encoder-decoder architectures have been shown empirically to excel on sequence to sequence tasks [@cho_learning_2014].
...
It is thought that by encoding the input sequence to a vector, encoder-decoders find informative representations of the input sequence that make the prediction task easier for the decoder [@sutskever_sequence_2014].
...
To compare forecasts, we focus exclusively on metrics of forecast skill which satisfy @gneiting_strictly_2007's property of a strictly proper score.
...
We examine forecast skill for each of the six forecasting methods (MCMC, ARIMA, block-RNN, GRU, LSTM, and transformer) in each of our three scenarios (Hopf bifurcation, saddle-node bifurcation, stochastic transition).
In addition to these cases, we also consider an "ensemble model", generated from drawing from the distribution of all models except the MCMC model. 
Such ensemble techniques can better reflect uncertainty than relying on any single method [@something_about_ensembles].
For simplicity, we consider the un-weighted case, where each model is represented equally in the ensemble.
Using model-based simulations allows us to examine performance against an multiple (n=100) replicates of the "true" process, which further helps identify differences that may occur solely due to chance. 
...
Note that in both bifurcation scenarios, future dynamics will see states never previously observed in the historical data that was used to train each of the methods (i.e. very small population sizes).  
...
Overall forecasting skill scores for each model across all three scenarios are summarized in figure 2.
...
Note that a wide prediction of uncertainty does not mean a wide range in the score skill -- for instance the ensemble model which has the widest array of outcomes often has the a relatively tight distribution of score, especially in logs skill.
This reflects the relative contributions of accuracy and uncertainty as components in the forecasts.
Overall, ML scores are comparable to MCMC skill except for the scenario of the saddle-node bifurcation, where all other models are much worse.
...
Forecasts of the Hopf bifurcation (Figure 3) are roughly comparable across the phenomenological models (ARIMA and machine learning models).
...
However, like the ML models, it fails to predict the growing amplitude of those oscillations.
...
The saddle node bifurcation proves even more difficult for most methods (Figure 4).
...
#| Patterns show forecast skill generally get worse over time -
#| gradually in the case of Hopf bifurcation or suddenly in repsonse to the saddle-node bifurcation.
...
#| Comparing this panel to those in Figure 6 highlights scenarios that most often underestimate uncertainty.
...
In this paper, we carry out an initial exploration on how deep learning methods can perform on predicting transition events.
We compare the ability of several cutting edge machine learning approaches against statistical and process-based models, and show that deep learning methods are able to strike a middle ground between what we consider as acceptable and ideal case forecasting methods, ARIMA and MCMC-based parameter estimation respectively.
When machine learning models are able to observe transition events, as in the stochastic case, they generally outperformed MCMC-based forecasting but under-performed MCMC when there were no transition events in the training sets as in the Hopf and Saddle-node examples.
Yet, there are nuances to these assessments of performance since forecasting skill varies over time as we show in @figureX.
For instance, while most of the ML methods outperform MCMC in the stochastic transition example when forecasting skill is averaged over all time points, MCMC produces more replicates with relatively small errors than the ML models.

There are a number of questions that we have left unanswered in our study that we hope will be addressed in future work.
In this paper, we have explored a small number of machine learning and statistical models that can be used for forecasting, so general conclusions can not be drawn on whether statistical or machine learning-based approaches are better suited for tipping point forecasting problems.
Neither can we claim that the ML methods employed will translate well to all transition event forecasting  problems in reality, since working with real data will introduce additional difficulties.
For instance, to train a neural network on more than one real time series, we would likely have to employ a specialized algorithm that would allow for generalization over varying time and state scales.

Our analysis has focused on the task of making a single forecast prior to the occurrence of a critical transition.
Forecasting is ideally a more iterative process of data assimilation, where forecasts are updated with respect to additional observations, rather than projecting 100s of time steps into the future [@Dietz2018]. 
...
We leave these cases to future exploration. 
...
This paper is not intended to give a comprehensive answer to whether machine learning models are the the best methods to use on any tipping point forecasting problem as this will take numbers of studies to resolve. 
Instead, this paper aims to serve as a first exploration on whether deep learning methods should be considered as viable tools for the extremely challenging category of no-analog bifurcation forecasting problems.
...
- Forecasts with 'assimilation' i.e. ability to re-estimate after seeing some of the data, rather than looking at a single forecast with a long horizon.
