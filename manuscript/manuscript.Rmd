...
forecasts |> filter(simulation=="hopf", reps == 1, group == 1) |> 
  ...
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha=0.5, data = hopf_obs) +
...
forecasts |> filter(simulation=="saddle", group == 1, reps == 1) |> 
  ...
  geom_ribbon(aes(ymin = pmax(lower,0), ymax = upper), alpha=0.5, data = saddle_obs) +
  geom_line(aes(y=mean)) +
...
forecasts |> filter(simulation=="stochastic", group == 1, reps==1) |> 
  ggplot(aes(t, col=type, fill=type)) + 
  geom_ribbon(aes(ymin = pmax(lower,0), ymax = upper), alpha=0.5) +
  geom_ribbon(aes(ymin = pmax(lower,0), ymax = upper), alpha=0.5, data = stochastic_obs) +
  geom_line(aes(y=mean)) +
...
p1 <- over_time |> filter(metric == "crps") |>
  ...
  geom_pointrange(aes(y = score, ymin = score - sd, ymax = score + sd),
